...
#' 
#' Given multiple landscape functions \eqn{\Lambda_1 (t), \Lambda_2 (t), \ldots, \Lambda_N (t)}, 
#' compute \eqn{L_p} distance in a pairwise sense where 
#' \deqn{\|\Lambda_i (t) - \Lambda_j (t) \|_p = \left( \sum_{k=1}^K \| \Lambda_{k,i} (t) - \Lambda_{k,j} (t) \|_p^p \right)^{1/p} } 
...
#' @param as.dist logical; if TRUE, it returns dist object, else it returns a symmetric matrix.
#' 
...
#' dmat0 = pldist(list_land0, p=2)
#' dmat1 = pldist(list_land1, p=2)
#' 
#' ## Visualize
...
#' @concept landscape
#' @export
pldist <- function(dlist, p=2, as.dist=FALSE){
  ## PREPROCESSING
  if (!check_list_landscape(dlist)){
    stop("* pldist : 'dlist' should be a list of 'landscape' objects.")
  }
  ...
  list3d = adjust_list_landscapes(dlist, as.list=FALSE)
