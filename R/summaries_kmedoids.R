...
#' perform k-medoids clustering using pairwise distances using \eqn{L_2} metric.
#' 
...
#' @concept summaries
#' @export
fskmedoids <- function(fslist, k=2){
  ## PREPROCESSING
  dtype    = check_list_summaries("fskmedoids", fslist)
  myk      = max(1, round(k))
  
  ## COMPUTE PAIRWISE DISTANCE
  distobj = TDAkit::fsdist(fslist, as.dist=TRUE)
  ...
  ## WRAP AND RETURN
  output = as.integer(obj.kmedoids$clustering)
  return(output)
}
